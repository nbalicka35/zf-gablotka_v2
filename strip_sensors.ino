...
//#define MOTION_SENSOR_PIN 13 /* @pin where to plug motion sensor */

  // DS1302 RTC pins
  const int kCePin   = 3;  // Chip Enable
  const int kIoPin   = 4;  // Input/Output (changed from 6 to avoid conflict with LED_PIN)
  const int kSclkPin = 5;  // Serial Clock (changed from 7 to avoid conflicts)
  
  // Create a DS1302 object
  DS1302 rtc(kCePin, kIoPin, kSclkPin);
  
  // Create NeoPixel objecte
...
void distance_checker(){
  ...
  else if (distance_in_cm > 40 && (distance_in_cm <= 150)){
...
void ligthing_intensity(float illuminance){
  // TODO: implement look-up table for natural light transition
