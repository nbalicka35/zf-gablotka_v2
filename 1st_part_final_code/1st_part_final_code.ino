...
/* @brief Emulates a breathing effect. 
  ...
  @param del - delay (in miliseconds) passed to the function, type unsigned integer
...
void breathe(uint32_t del, uint32_t color){
  ...
  for(int i=1; i < (BRIGHTNESS_VALUE / 10) + 1; i++){
    ...
    // Replace delay with pure iteration
    volatile unsigned long counter = 0;
    // Calibrated for 48 MHz microcontroller
    // Higher numbers will create longer delays
    const unsigned long iterationsFor50ms = 300000;
    for(counter = 0; counter < iterationsFor50ms; counter++) {
      // Empty loop to consume time 
      // The volatile keyword prevents optimization
    }
  }
  
  /* This delay is required to emulate breathing effect. REPLACED with iteration */
  volatile unsigned long bigCounter = 0;
  // Calibrated for 48 MHz microcontroller
  // This attempts to approximate the original delay in milliseconds
  const unsigned long iterationsPerMs = 6000;
  const unsigned long totalIterations = del * iterationsPerMs;
  
  for(bigCounter = 0; bigCounter < totalIterations; bigCounter++) {
    // Empty loop to consume time
    // The volatile keyword prevents optimization
  }
